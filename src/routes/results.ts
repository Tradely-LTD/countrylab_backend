import { Router, Request, Response } from "express";
import { z } from "zod";
import { eq, and, desc, sql } from "drizzle-orm";
import { db } from "../db";
import {
  results,
  samples,
  result_changes,
  users,
  notifications,
} from "../db/schema";
import {
  authenticate,
  requireRole,
  LAB_ROLES,
  APPROVAL_ROLES,
} from "../middleware/auth";
import { createAuditLog } from "../middleware/audit";
import { AppError } from "../middleware/errorHandler";
import { generateCoaPdf } from "../services/pdfService";
import { generateQRCode, generateQRHash } from "../services/qrService";
import { sendNotification } from "../services/notificationService";
import { logger } from "../utils/logger";
import crypto from "crypto";

const router = Router();

// ─── Validation ───────────────────────────────────────────────────────────────

const parameterSchema = z.object({
  param_name: z.string().min(1, "Parameter name is required"),
  raw_value: z.union([z.string(), z.number()]).optional().nullable(),
  calculated_value: z.union([z.string(), z.number()]).optional().nullable(),
  unit: z.string().optional().nullable(),
  spec_min: z.number().optional().nullable(),
  spec_max: z.number().optional().nullable(),
  pass: z.boolean().optional(),
  warning: z.boolean().optional().default(false),
  data_type: z.enum(["numerical", "qualitative"]).default("numerical"),
});

const createResultSchema = z.object({
  sample_id: z.string().uuid(),
  test_method_id: z.string().uuid().optional(),
  parameters: z.array(parameterSchema).min(1),
  notes: z.string().optional(),
});

const updateResultSchema = z.object({
  parameters: z.array(parameterSchema).optional(),
  notes: z.string().optional(),
  reason_for_change: z
    .string()
    .min(10, "Please provide a detailed reason for the change"),
});

// ─── GET /results ─────────────────────────────────────────────────────────────

router.get("/", authenticate, async (req: Request, res: Response) => {
  const {
    page = "1",
    limit = "25",
    status,
    sample_id,
  } = req.query as Record<string, string>;
  const offset = (parseInt(page) - 1) * parseInt(limit);

  const conditions = [eq(results.tenant_id, req.tenantId!)];
  if (status) conditions.push(eq(results.overall_status, status as any));
  if (sample_id) conditions.push(eq(results.sample_id, sample_id));

  const [resultList, [{ count }]] = await Promise.all([
    db
      .select({
        id: results.id,
        overall_status: results.overall_status,
        coa_url: results.coa_url,
        qr_hash: results.qr_hash,
        approved_at: results.approved_at,
        created_at: results.created_at,
        sample: {
          id: samples.id,
          ulid: samples.ulid,
          name: samples.name,
          matrix: samples.matrix,
        },
        analyst: {
          id: users.id,
          full_name: users.full_name,
        },
      })
      .from(results)
      .leftJoin(samples, eq(results.sample_id, samples.id))
      .leftJoin(users, eq(results.analyst_id, users.id))
      .where(and(...conditions))
      .orderBy(desc(results.created_at))
      .limit(parseInt(limit))
      .offset(offset),

    db
      .select({ count: sql<number>`count(*)::int` })
      .from(results)
      .where(and(...conditions)),
  ]);

  res.json({
    data: resultList,
    pagination: { page: parseInt(page), limit: parseInt(limit), total: count },
  });
});

// ─── GET /results/:id ─────────────────────────────────────────────────────────

router.get("/:id", authenticate, async (req: Request, res: Response) => {
  const [result] = await db
    .select({
      id: results.id,
      tenant_id: results.tenant_id,
      sample_id: results.sample_id,
      test_method_id: results.test_method_id,
      analyst_id: results.analyst_id,
      approver_id: results.approver_id,
      parameters: results.parameters,
      notes: results.notes,
      overall_status: results.overall_status,
      qr_hash: results.qr_hash,
      qr_code_url: results.qr_code_url,
      coa_url: results.coa_url,
      locked_at: results.locked_at,
      voided_at: results.voided_at,
      approved_at: results.approved_at,
      reviewed_at: results.reviewed_at,
      created_at: results.created_at,
      updated_at: results.updated_at,
      sample: {
        id: samples.id,
        ulid: samples.ulid,
        name: samples.name,
        matrix: samples.matrix,
        collection_date: samples.collection_date,
      },
      analyst: {
        id: users.id,
        full_name: users.full_name,
        email: users.email,
        role: users.role,
      },
    })
    .from(results)
    .leftJoin(samples, eq(results.sample_id, samples.id))
    .leftJoin(users, eq(results.analyst_id, users.id))
    .where(
      and(eq(results.id, req.params.id), eq(results.tenant_id, req.tenantId!)),
    )
    .limit(1);

  if (!result) throw new AppError(404, "Result not found");

  // Get change history
  const changes = await db
    .select()
    .from(result_changes)
    .where(eq(result_changes.result_id, result.id))
    .orderBy(desc(result_changes.created_at));

  res.json({ data: { ...result, changes } });
});

// ─── POST /results ────────────────────────────────────────────────────────────

router.post(
  "/",
  authenticate,
  requireRole(...LAB_ROLES),
  async (req: Request, res: Response) => {
    const body = createResultSchema.parse(req.body);

    // Verify sample belongs to tenant
    const [sample] = await db
      .select()
      .from(samples)
      .where(
        and(
          eq(samples.id, body.sample_id),
          eq(samples.tenant_id, req.tenantId!),
        ),
      )
      .limit(1);

    if (!sample) throw new AppError(404, "Sample not found");
    if (sample.status === "approved")
      throw new AppError(400, "Sample already has an approved result");

    // Check for outliers
    const parameters = body.parameters.map((p) => {
      if (
        p.data_type === "numerical" &&
        typeof p.calculated_value === "number"
      ) {
        const hasMin = p.spec_min !== undefined;
        const hasMax = p.spec_max !== undefined;
        const belowMin = hasMin && p.calculated_value < p.spec_min!;
        const aboveMax = hasMax && p.calculated_value > p.spec_max!;
        const warning = belowMin || aboveMax;
        const pass = !warning;
        return { ...p, warning, pass };
      }
      return p;
    });

    const [newResult] = await db
      .insert(results)
      .values({
        tenant_id: req.tenantId!,
        sample_id: body.sample_id,
        test_method_id: body.test_method_id,
        analyst_id: req.user!.id,
        parameters: parameters as any,
        notes: body.notes,
        overall_status: "draft",
      })
      .returning();

    // Update sample status
    await db
      .update(samples)
      .set({ status: "in_testing", updated_at: new Date() })
      .where(eq(samples.id, body.sample_id));

    await createAuditLog({
      tenant_id: req.tenantId!,
      user_id: req.user!.id,
      action: "CREATE",
      table_name: "results",
      record_id: newResult.id,
      new_value: newResult as unknown as Record<string, unknown>,
    });

    res.status(201).json({
      data: newResult,
      message: "Result created successfully",
      hasOutliers: parameters.some((p) => p.warning),
    });
  },
);

// ─── PUT /results/:id ─────────────────────────────────────────────────────────

router.put(
  "/:id",
  authenticate,
  requireRole(...LAB_ROLES),
  async (req: Request, res: Response) => {
    const body = updateResultSchema.parse(req.body);

    const [existing] = await db
      .select()
      .from(results)
      .where(
        and(
          eq(results.id, req.params.id),
          eq(results.tenant_id, req.tenantId!),
        ),
      )
      .limit(1);

    if (!existing) throw new AppError(404, "Result not found");
    if (existing.locked_at)
      throw new AppError(400, "Result is locked after approval");
    if (existing.voided_at)
      throw new AppError(400, "Cannot modify a voided result");

    // Log the change
    await db.insert(result_changes).values({
      result_id: existing.id,
      changed_by: req.user!.id,
      reason: body.reason_for_change,
      old_parameters: existing.parameters as any,
      new_parameters: body.parameters as any,
    });

    const [updated] = await db
      .update(results)
      .set({
        parameters: body.parameters as any,
        notes: body.notes,
        overall_status: "draft", // Reset to draft on edit
        updated_at: new Date(),
      })
      .where(eq(results.id, req.params.id))
      .returning();

    await createAuditLog({
      tenant_id: req.tenantId!,
      user_id: req.user!.id,
      action: "UPDATE",
      table_name: "results",
      record_id: existing.id,
      old_value: { parameters: existing.parameters },
      new_value: { parameters: body.parameters },
      metadata: { reason: body.reason_for_change },
    });

    res.json({ data: updated });
  },
);

// ─── POST /results/:id/submit ─────────────────────────────────────────────────

router.post(
  "/:id/submit",
  authenticate,
  requireRole(...LAB_ROLES),
  async (req: Request, res: Response) => {
    const [existing] = await db
      .select()
      .from(results)
      .where(
        and(
          eq(results.id, req.params.id),
          eq(results.tenant_id, req.tenantId!),
        ),
      )
      .limit(1);

    if (!existing) throw new AppError(404, "Result not found");
    if (existing.overall_status !== "draft") {
      throw new AppError(
        400,
        `Result is already in '${existing.overall_status}' status`,
      );
    }

    const [updated] = await db
      .update(results)
      .set({ overall_status: "submitted", updated_at: new Date() })
      .where(eq(results.id, req.params.id))
      .returning();

    // Update sample status
    await db
      .update(samples)
      .set({ status: "pending_review", updated_at: new Date() })
      .where(eq(samples.id, existing.sample_id));

    // Notify MD/approvers
    await sendNotification({
      tenant_id: req.tenantId!,
      type: "RESULT_PENDING_REVIEW",
      title: "Result Submitted for Review",
      message: `A result has been submitted and awaits your approval.`,
      roles: ["md", "super_admin"],
      link: `/results/${existing.id}`,
    });

    await createAuditLog({
      tenant_id: req.tenantId!,
      user_id: req.user!.id,
      action: "UPDATE",
      table_name: "results",
      record_id: existing.id,
      new_value: { status: "submitted" },
      metadata: { action: "SUBMITTED_FOR_REVIEW" },
    });

    res.json({ data: updated, message: "Result submitted for review" });
  },
);

// ─── POST /results/:id/approve ────────────────────────────────────────────────

router.post(
  "/:id/approve",
  authenticate,
  requireRole(...APPROVAL_ROLES),
  async (req: Request, res: Response) => {
    const [existing] = await db
      .select()
      .from(results)
      .where(
        and(
          eq(results.id, req.params.id),
          eq(results.tenant_id, req.tenantId!),
        ),
      )
      .limit(1);

    if (!existing) throw new AppError(404, "Result not found");
    if (
      !["submitted", "under_review"].includes(existing.overall_status || "")
    ) {
      throw new AppError(400, "Result must be submitted before approval");
    }

    const now = new Date();
    const qrHash = generateQRHash(existing.id);
    const qrCodeUrl = await generateQRCode(qrHash, req.tenantId!);

    // Generate CoA PDF
    const coaUrl = await generateCoaPdf({
      result_id: existing.id,
      tenant_id: req.tenantId!,
      qr_hash: qrHash,
    });

    const [updated] = await db
      .update(results)
      .set({
        overall_status: "approved",
        approver_id: req.user!.id,
        approved_at: now,
        locked_at: now,
        qr_hash: qrHash,
        qr_code_url: qrCodeUrl,
        coa_url: coaUrl,
        updated_at: now,
      })
      .where(eq(results.id, req.params.id))
      .returning();

    // Update sample status to approved
    await db
      .update(samples)
      .set({ status: "approved", updated_at: now })
      .where(eq(samples.id, existing.sample_id));

    await createAuditLog({
      tenant_id: req.tenantId!,
      user_id: req.user!.id,
      action: "APPROVE",
      table_name: "results",
      record_id: existing.id,
      metadata: { action: "APPROVED", qr_hash: qrHash },
    });

    res.json({
      data: updated,
      message: "Result approved. CoA has been generated.",
    });
  },
);

// ─── POST /results/:id/reject ─────────────────────────────────────────────────

router.post(
  "/:id/reject",
  authenticate,
  requireRole(...APPROVAL_ROLES),
  async (req: Request, res: Response) => {
    const { reason } = z.object({ reason: z.string().min(10) }).parse(req.body);

    const [updated] = await db
      .update(results)
      .set({ overall_status: "rejected", updated_at: new Date() })
      .where(
        and(
          eq(results.id, req.params.id),
          eq(results.tenant_id, req.tenantId!),
        ),
      )
      .returning();

    if (!updated) throw new AppError(404, "Result not found");

    await createAuditLog({
      tenant_id: req.tenantId!,
      user_id: req.user!.id,
      action: "REJECT",
      table_name: "results",
      record_id: updated.id,
      metadata: { reason },
    });

    res.json({ data: updated, message: "Result rejected" });
  },
);

// ─── GET /results/:id/coa (Download CoA PDF) ──────────────────────────────────

router.get("/:id/coa", authenticate, async (req: Request, res: Response) => {
  const [result] = await db
    .select()
    .from(results)
    .where(
      and(eq(results.id, req.params.id), eq(results.tenant_id, req.tenantId!)),
    )
    .limit(1);

  if (!result) throw new AppError(404, "Result not found");
  if (result.overall_status !== "approved")
    throw new AppError(400, "CoA only available for approved results");
  if (!result.coa_url) throw new AppError(404, "CoA not yet generated");

  res.json({ data: { coa_url: result.coa_url, qr_hash: result.qr_hash } });
});

// ─── GET /verify/:qrHash (PUBLIC — no auth) ───────────────────────────────────

router.get("/verify/:qrHash", async (req: Request, res: Response) => {
  const [result] = await db
    .select({
      id: results.id,
      overall_status: results.overall_status,
      approved_at: results.approved_at,
      parameters: results.parameters,
      qr_hash: results.qr_hash,
      qr_code_url: results.qr_code_url,
      sample: {
        ulid: samples.ulid,
        name: samples.name,
        matrix: samples.matrix,
        collection_date: samples.collection_date,
      },
    })
    .from(results)
    .leftJoin(samples, eq(results.sample_id, samples.id))
    .where(eq(results.qr_hash, req.params.qrHash))
    .limit(1);

  if (!result) {
    return res.json({
      verified: false,
      message: "No record found. This report may be fraudulent.",
    });
  }

  res.json({
    verified: true,
    message: "Authentic Report — Verified by Countrylab",
    data: {
      sample: result.sample,
      approved_at: result.approved_at,
      status: result.overall_status,
      parameters: result.parameters,
      qr_code_url: result.qr_code_url,
      qr_hash: result.qr_hash,
    },
  });
});

// ─── GET /results/meta/queue (MD approval queue) ──────────────────────────────

router.get(
  "/meta/queue",
  authenticate,
  requireRole(...APPROVAL_ROLES),
  async (req: Request, res: Response) => {
    const queue = await db
      .select({
        id: results.id,
        overall_status: results.overall_status,
        created_at: results.created_at,
        notes: results.notes,
        sample: {
          id: samples.id,
          ulid: samples.ulid,
          name: samples.name,
          matrix: samples.matrix,
        },
        analyst: {
          id: users.id,
          full_name: users.full_name,
        },
      })
      .from(results)
      .leftJoin(samples, eq(results.sample_id, samples.id))
      .leftJoin(users, eq(results.analyst_id, users.id))
      .where(
        and(
          eq(results.tenant_id, req.tenantId!),
          sql`${results.overall_status} IN ('submitted', 'under_review')`,
        ),
      )
      .orderBy(results.created_at);

    res.json({ data: queue, count: queue.length });
  },
);

// ─── POST /results/:id/regenerate-qr ──────────────────────────────────────────

router.post(
  "/:id/regenerate-qr",
  authenticate,
  requireRole(...APPROVAL_ROLES),
  async (req: Request, res: Response) => {
    const [existing] = await db
      .select()
      .from(results)
      .where(
        and(
          eq(results.id, req.params.id),
          eq(results.tenant_id, req.tenantId!),
        ),
      )
      .limit(1);

    if (!existing) throw new AppError(404, "Result not found");
    if (existing.overall_status !== "approved") {
      throw new AppError(
        400,
        "Only approved results can have QR codes regenerated",
      );
    }

    const qrHash = existing.qr_hash || generateQRHash(existing.id);
    const qrCodeUrl = await generateQRCode(qrHash, req.tenantId!);

    const [updated] = await db
      .update(results)
      .set({
        qr_hash: qrHash,
        qr_code_url: qrCodeUrl,
        updated_at: new Date(),
      })
      .where(eq(results.id, req.params.id))
      .returning();

    res.json({
      data: updated,
      message: "QR code regenerated successfully",
      qr_code_url: qrCodeUrl,
    });
  },
);

// ─── DELETE /results/:id ──────────────────────────────────────────────────────

router.delete(
  "/:id",
  authenticate,
  requireRole(...LAB_ROLES),
  async (req: Request, res: Response) => {
    const [existing] = await db
      .select()
      .from(results)
      .where(
        and(
          eq(results.id, req.params.id),
          eq(results.tenant_id, req.tenantId!),
        ),
      )
      .limit(1);

    if (!existing) throw new AppError(404, "Result not found");

    // Only allow deletion of draft results
    if (existing.overall_status !== "draft") {
      throw new AppError(400, "Only draft results can be deleted");
    }

    // Delete the result
    await db.delete(results).where(eq(results.id, req.params.id));

    await createAuditLog({
      tenant_id: req.tenantId!,
      user_id: req.user!.id,
      action: "DELETE",
      table_name: "results",
      record_id: existing.id,
      old_value: existing as unknown as Record<string, unknown>,
    });

    res.json({ message: "Result deleted successfully" });
  },
);

export default router;
