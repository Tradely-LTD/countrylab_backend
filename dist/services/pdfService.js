...
exports.generateCoaPdf = generateCoaPdf;
...
async function generateCoaPdf(data) {
    try {
        ...
        const [result] = await db_1.db
            ...
            .where((0, drizzle_orm_1.eq)(schema_1.results.id, data.result_id))
            .limit(1);
        const [sample] = await db_1.db
            .select()
            .from(schema_1.samples)
            .where((0, drizzle_orm_1.eq)(schema_1.samples.id, result.sample_id))
            .limit(1);
        const [client] = await db_1.db
            .select()
            .from(schema_1.clients)
            .where((0, drizzle_orm_1.eq)(schema_1.clients.id, sample.client_id))
            .limit(1);
        const [tenant] = await db_1.db
            .select()
            .from(schema_1.tenants)
            .where((0, drizzle_orm_1.eq)(schema_1.tenants.id, data.tenant_id))
            .limit(1);
        const [analyst] = await db_1.db
            .select()
            .from(schema_1.users)
