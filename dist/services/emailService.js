...
exports.sendRequestApprovalEmail = sendRequestApprovalEmail;
...
async function sendRequestConfirmation(data) {
    try {
        const transporter = getTransporter();
        if (!transporter) {
            logger_1.logger.warn("Email transporter not available, skipping email");
            return false;
        }
        const { requestNumber, clientName, clientEmail, representativeName, representativeEmail, productName, testCategory, } = data;
        // Email to client
      ...
      `,
        ...
        logger_1.logger.info(`Request confirmation emails sent for ${requestNumber}`);
        return true;
    }
    catch (error) {
        logger_1.logger.error("Failed to send request confirmation emails:", error);
        // Don't throw error - email failure shouldn't block request creation
        return false;
    }
}
async function sendRequestApprovalEmail(data) {
    try {
        const transporter = getTransporter();
        if (!transporter) {
            logger_1.logger.warn("Email transporter not available, skipping email");
      ...
      `,
        ...
        logger_1.logger.info(`Approval email sent for ${requestNumber}`);
        return true;
    }
    catch (error) {
        logger_1.logger.error("Failed to send approval email:", error);
        return false;
    }
}
